...
def run(fh):
    ...
    bested_hands = []
    for hand in hands:
        bested_hands.append((hand_find_best(hand[0]), hand[1], hand[0]))

    hands = bested_hands
    print(f"bested_hands: {hands}")

    hands = [(hand[0], hand[1], hand_type(hand[0]), hand[2]) for hand in hands]
    for hand in hands:
        print(f"Hand: {hand}")
...
def hand_compare(th1, th2):
    ...
    o1, o2 = th1[3], th2[3]
    assert o1 != o2, f"Hands are the same: {th1} == {th2}"
    if v1 > v2:
        return -1
    elif v1 < v2:
        return 1
        ...
        for c in range(5):
            if card_order.index(o1[c]) > card_order.index(o2[c]):
                return -1
            elif card_order.index(o1[c]) < card_order.index(o2[c]):
                return 1
...
def hand_find_best(h: str):
    # print(f" hand_find_best: {h}")
    if not "J" in h:
        return h
    best_hand = h.replace("J", "2")
    for C in card_order:
        potential_hand = h.replace("J", C)
        if hand_type(potential_hand).value < hand_type(best_hand).value:
